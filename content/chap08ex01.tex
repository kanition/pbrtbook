...
\begin{remark}
    ...
    \citet{1138991}、\cite{1142437}和\citet{10.5555/2383847.2383874}整理
    并推导后补充的，请酌情参考和斧正。
\end{remark}
...
通过平稳的随机过程叠加上或正或负的高度波动得到的。在如\reffig{08ex01-G1Geometry}所示，
我们继续分析一个微观区域：仍把宏曲面视作平面且就是$z=0$，宏曲面法线为${\bm\omega}_{\mathrm{g}}=(0,0,1)$。
...
此外，我们图示中考虑的出射方向大致向右，所以不失一般性，我们假定只有点$A$右边的微曲面才有可能挡住出射光线。
...
那微曲面经过截平面的截取后，点$A$处在被截出的曲线上的一维斜率$q_A$
...
\begin{figure}[htbp]
    ...
    \caption{要求得微曲面上点$A$能从${\bm\omega}_{\mathrm{o}}$方向观察到的概率，
    可以先求对应光线在极小区间$[t,t+\Delta t]$内与微曲面相交的条件概率，再作转化。}
    \label{fig:08ex01-G1Geometry}
\end{figure}

接下来的问题关键是怎么求出上式关于$g(t)$的积分。
...
显然只要微曲面高度$h$低于光线高度$h_A+\mu t$即可：
\begin{align}
    h<h_A+\mu t\, ,
\end{align}
我们把这一事件记作$X$，利用高度概率分布$P_H(h)$算出相应的概率为
\begin{align}
    P(X)=\int_{-\infty}^{h_A+\mu t}P_H(h)\mathrm{d}h\, .
\end{align}
...
事件$X$和$Y$同时发生的概率为
\begin{align}
    P(X,Y)=\int_{q=\mu}^{+\infty}\int_{h=h_A+\mu t-(q-\mu)\Delta t}^{h_A+\mu t}
    P_{\mathcal{M}}(h,q|A,t)\mathrm{d}h\mathrm{d}q\, ,
\end{align}
其中$P_{\mathcal{M}}(h,q|A,t)$是给定点$A$和位置$t$的前提下，
微曲面上$t$处的高度$h$和一维斜率$q$的联合概率密度函数。
通常我们假设高度$h$和一维斜率$q$作为随机变量是相互独立的，即
\begin{align}
    P_{\mathcal{M}}(h,q|A,t)=P_H(h)P_s(q)\, .
\end{align}
于是$g(t)\Delta t$就是事件$Y$对事件$X$的条件概率
\begin{align}
...
\begin{align}\label{eq:08ex01-gt-expression}
    ...
    =\lim\limits_{\Delta t\to 0}\frac{\displaystyle\int_{q=\mu}^{+\infty}\int_{h=h_A+\mu t-(q-\mu)\Delta t}^{h_A+\mu t}
    P_{\mathcal{M}}(h,q|A,t)\mathrm{d}h\mathrm{d}q}{\Delta t\displaystyle\int_{-\infty}^{h_A+\mu t}P_H(h)\mathrm{d}h}\nonumber\\
    =&\frac{\displaystyle\int_{q=\mu}^{+\infty}(q-\mu)P_s(q)\lim\limits_{\Delta t\to 0}
    \left(\frac{1}{(q-\mu)\Delta t}\int_{h=h_A+\mu t-(q-\mu)\Delta t}^{h_A+\mu t}P_H(h)\mathrm{d}h\right)\mathrm{d}q}
...
\end{align}
我们在所有被观察点$A$的高度$h_A$上积分，并再次利用$F_H$和$P_H$之间的微分关系，
就得到整个微曲面上对该光线的平均可见概率为
\begin{align}
    W(u_A,v_A,{\bm\omega}_{\mathrm{o}})&=\int_{-\infty}^{+\infty}W(A,{\bm\omega}_{\mathrm{o}})P_H(h_A)\mathrm{d}h_A\nonumber\\
    =&\chi(\mu-q_A)\int_{-\infty}^{+\infty}F_H^{\Lambda(\mu)}(h_A)P_H(h_A)\mathrm{d}h_A\nonumber\\
    =&\chi(\mu-q_A)\int_{-\infty}^{+\infty}F_H^{\Lambda(\mu)}(h_A)\mathrm{d}F_H(h_A)\nonumber\\
    =&\frac{\chi(\mu-q_A)}{1+\Lambda(\mu)}F_H^{1+\Lambda(\mu)}(h_A)\bigg|_{h_A=-\infty}^{+\infty}\nonumber\\
...
由于前面的推导都是严格等式，没有近似项，这说明在非自相关的假设下，Smith掩模函数是准确的。
