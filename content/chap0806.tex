...
前\refvar[FourierBSDF::bsdfTable]{bsdfTable}{}{\ttfamily .}\refvar[FourierBSDFTable::mMax]{mMax}{}个项
会用于第一个通道，接下来的\refvar[FourierBSDFTable::mMax]{mMax}{}个项用于第二个通道，以此类推
（因此在十六个方向上的最大阶数小于\refvar[FourierBSDFTable::mMax]{mMax}{}的常见情况下，
通常数组{\ttfamily ak}中会有一些没用到的空间）。
所有这些存储都初始化为零，所以后续代码可以直接把\refeq{8.22}的项加到{\ttfamily ak}的对应项中。
\begin{lstlisting}
