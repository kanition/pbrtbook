...
\sidenote{译者注：更严格地说，是用于计算微面微分面积的因子。
    原文把它以及后文多个类似的因子直接称为“面积”起到了简化理解的作用，
    但笔者认为有必要向读者澄清这一点，请读者注意区分。}
...
对应的各向异性微面分布函数为
\sidenote{译者注：笔者补充编写了\refsec{译者补充：两个微面分布函数的规范性证明}，
    给出了本节微面分布模型满足规范性的证明。}
...
\sidenote{译者注：用公式表达的话，记$H^2({\bm n})$内
满足${\bm\omega}\cdot{\bm\omega}_{\mathrm{h}}\ge0$的部分为$P$，
不满足的部分为$N$，则$A^{+}({\bm\omega})$和$A^{-}({\bm\omega})$分别是
在$P$和$N$上的积分值$\displaystyle\int({\bm\omega}\cdot{\bm\omega}_{\mathrm{h}})D({\bm\omega}_{\mathrm{h}})\mathrm{d}{\bm\omega}_{\mathrm{h}}$.}，
则$\cos\theta=A^{+}({\bm\omega})-A^{-}({\bm\omega})$.
因此我们可以把掩模遮挡函数改写为可见微面面积与正向微面面积之比
\sidenote{译者注：读者可以将其视作$G_1({\bm\omega})$的数学定义，
它从$G_1({\bm\omega},{\bm\omega}_{\mathrm{h}})$继续推导而来，
而不必太纠结它与后者的“简化关系”。}：
\begin{align*}
