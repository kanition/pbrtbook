...
\begin{prove}
    考虑$\theta\in[0,\pi]$的情况\sidenote{思考一下：为什么只需要考虑这个角度区间？超过这个区间又该如何处理？}，
...
选最大的一项开方\sidenote{思考一下：开方后取正负会有影响吗？不直接全部开方求解的原因除了开销大外还有别的吗？}，
...
\begin{enumerate}
    ...
    \item 取最大的那项，按\refeq{02ex.2}对应行计算该项并开方；
    \item 计算$\bm R-\bm R^\mathrm{T}$或$\bm R+\bm R^\mathrm{T}$，
          分别用所得矩阵的元素（\refeq{02ex.1}）除以上一步的已知项并乘以系数$\displaystyle\frac{1}{4}$得到剩余未知项。
\end{enumerate}