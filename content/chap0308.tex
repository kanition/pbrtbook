...
\sidenote{译者注：原图没有正确显示脚标，已修正。}。
\begin{figure}[htbp]
    \centering\includegraphics[width=0.3\linewidth]{chap03/Subdivedgetangents.eps}
    \caption{边界顶点的切线也计算为相邻顶点的加权平均。
        然而，一些边界切线规则并入了中心顶点的值。}
    \label{fig:3.38}
\end{figure}

第一条切线称为\keyindex{跨越切线}{across tangent}{tangent line切线}，
由两个相邻边界顶点之间的向量给出：
\begin{align*}
    \bm s=\bm v_{n-1}-\bm v_0\, .
\end{align*}

第二条切线称为\keyindex{横断切线}{transverse tangent}{tangent line切线}，
是基于顶点的价算得的。
