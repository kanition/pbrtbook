...
两个面的公共边须由每个面指定为不同方向。
考虑两个顶点$\bm v_0$和$\bm v_1$以及连接它们的边。
我们希望一个拥有该边的三角形面指定其三个顶点时$\bm v_0$在$\bm v_1$之前，
另一个面指定顶点时$\bm v_1$在$\bm v_0$之前（\reffig{3.28}）。
\keyindex{莫比乌斯带}{Möbius strip}{}是一个无法相容次序的曲面例子，
但渲染中几乎不会遇到这样的曲面，所以实践中该限制没有问题。
然而不能创建相容次序网格的程序给出的构建得很差的网格数据可能会造成问题。
\begin{figure}[htbp]
