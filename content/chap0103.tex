...
以供在其上生成必需的样本来渲染图像。
计算{\ttfamily nTiles}时加上{\ttfamily tileSize - 1}使得
...
Bounds2i `\initvar{sampleBounds}{}` = `\refvar{camera}{}`->`\refvar{film}{}`->`\refvar{GetSampleBounds}{}`();
Vector2i `\initvar{sampleExtent}{}` = sampleBounds.`\refvar{Diagonal}{}`();
...
\refvar{MemoryArena}{}的实例管理内存池来启用比标准库例程更高性能的分配。

arena\sidenote{译者注：arena，竞技场。}的内存池总是整体释放，
消解了对复杂内部数据结构的需求。
该类的实例只能用于单个线程——不允许没有额外同步的并行存取。
我们为每个循环迭代创建可直接使用的唯一的\refvar{MemoryArena}{}，
也保证了arena只被单个线程访问。
\begin{lstlisting}
...
多数\refvar{Sampler}{}实现发现保持一些状态很有用，
例如当前渲染的像素坐标。
这意味着多处理线程不能兼用单个{\ttfamily Sampler}。
因此，{\ttfamily Sampler}提供了方法{\ttfamily Clone()}来
创建给定{\ttfamily Sampler}的新实例；
它取用的种子也用于伪随机数生成器的一些实现，
这样每个图块就不会生成一样的伪随机数序列。
（注意不是所有{\ttfamily Sampler}都使用伪随机数；
不需要会忽略种子。）
\begin{lstlisting}
`\initcode{Get sampler instance for tile}{=}`
int seed = tile.y * nTiles.x + tile.x;
std::unique_ptr<`\refvar{Sampler}{}`> tileSampler = sampler->`\refvar{Clone}{}`(seed);
\end{lstlisting}
...
Bounds2i tileBounds(`\refvar{Point2i}{}`(x0, y0), `\refvar{Point2i}{}`(x1, y1));
\end{lstlisting}
...
std::unique_ptr<FilmTile> filmTile =
    `\refvar{camera}{}`->`\refvar{film}{}`->`\refvar{GetFilmTile}{}`(tileBounds);
\end{lstlisting}
