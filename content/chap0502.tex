...
红、绿、蓝各一种，由显示器的磷光体\sidenote{译者注：原文phosphor。}发出，例如
LED\sidenote{译者注：即light-emitting diode，发光二极管。}
...
\begin{figure}[htbp]
    ...
    甚至给定相同RGB值时得到的发射SPD也明显不同，因为\reffig{5.4}中所示的发射曲线不同。}
    \label{fig:5.5}
\end{figure}

给定SPD的表示$(x_{\lambda},y_{\lambda},z_{\lambda})$，我们可以将其转换为相应的RGB系数，
