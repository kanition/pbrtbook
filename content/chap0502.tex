...
用方法\refvar{ToRGBSpectrum}{()}也能很容易地创建\refvar{RGBSpectrum}{}。
\begin{lstlisting}
`\refcode{SampledSpectrum Public Methods}{+=}\lastnext{SampledSpectrumPublicMethods}`
...
这里我们实现了\citet{doi:10.1080/10867651.1999.10487511}提出的将RGB转换为SPD
并尝试实现以上目标的的方法。
...
用常数SPD表示常数光谱更好。
第二，像黄色（红绿混合）那样由两种基色混合的颜色，
用它们预先计算的光滑SPD表示比用两种相应基色的SPD之和表示更好。
...
方法\refvar[FromRGB]{SampledSpectrum::FromRGB}{()}将给定RGB值转换为完整的SPD。
...
`\refvar{SampledSpectrum}{}` `\refvar{SampledSpectrum}{}`::`\initvar{FromRGB}{}`(const `\refvar{Float}{}` rgb[3],
...
`\refvar{SampledSpectrum}{}`::`\refvar{SampledSpectrum}{}`(const `\refvar{RGBSpectrum}{}` &r, `\refvar{SpectrumType}{}` t) {
    `\refvar{Float}{}` rgb[3];
    r.`\refvar{ToRGB}{}`(rgb);
    *this = `\refvar{SampledSpectrum}{}`::`\refvar{FromRGB}{}`(rgb, t);
}
\end{lstlisting}