...
现在可以进行渲染了。
用自动使用由类\refvar{Bounds2}{}提供的迭代器
的范围{\ttfamily for}循环对图块中所有像素遍历。
...
结构体\refvar{CameraSample}{}记录了相机应生成光线的相应胶片位置。
它还存储了时间和透镜位置采样值，
...
让计算纹理随像素空间变化有多快成为可能，这是纹理反锯齿的关键因素。
...
注意大写浮点类型\refvar{Float}{}：取决于pbrt的编译选项，
它是{\ttfamily float}或{\ttfamily double}的别名。
\refsec{主要包含文件}提供了这些设计选项的更多细节。

给定一条光线，下一个任务是确定沿其到达图像平面的辐射。
方法\refvar{Li}{()}负责这项任务。
\begin{lstlisting}
...
\begin{itemize}
    ...
    \item {\ttfamily arena}：为积分器高效临时分配内存的\refvar{MemoryArena}{}。
          ...
          因此不应使用{\ttfamily arena}分配任何比当前光线所需还驻留更久的内存。
    \item {\ttfamily depth}：光线从相机出发直到当前调用\refvar{Li}{()}之间反射的次数。
\end{itemize}
...
渲染过程中bug的一个常见副作用是计算出不可能的辐射值。
例如，除以零得到的辐射值等于IEEE浮点无穷或“not a number”值。
渲染器寻找这种可能情况，以及具有负数贡献的光谱，
并在遇到这些情况时打印错误信息。
此处我们不会包含执行这些的
...
来表示交点处的BSDF的解释详见\refsub{BSDF内存管理}。）
\begin{lstlisting}
...
Whitted积分器工作时递归地计算沿反射和折射光线方向的辐射。
...
它能表示来自天空的光照。
方法\refvar[Le]{Light::Le}{()}允许这样的光源返回其沿给定光线的辐射。
\begin{lstlisting}
...
\begin{figure}
    ...
    \caption{Whitted积分器的几何设置。
        ...
        我们要计算反射辐射的方向为${\bm \omega}_\mathrm{o}$；
        它是指向与入射光相反方向的向量。}
    \label{fig:1.20}
\end{figure}