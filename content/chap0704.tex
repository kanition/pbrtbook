...
\sidenote{译者注：得名自荷兰数学家Johannes Gaultherus van der Corput (1890-1975)。}，
它是由2进制倒根函数给出的1D序列：
\begin{align*}
    x_a=\varPhi_2(a)\, .
\end{align*}
...
Hammersley和Halton序列的缺点是随着基$b$增加，样本值会展现出惊人的规律模式。
该问题可以用\keyindex{置乱}{scrambled}{}
Halton和Hammersley序列解决，
其计算倒根时对数字施加了重排。
\begin{align}
    \label{eq:7.8}
