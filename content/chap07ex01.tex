...
\begin{definition}
    数学中，\keyindex{狄拉克$\delta$分布}{Dirac delta distribution}{}
    是定义在实数域上的广义分布。
    它在除零以外的点上都取零，且在整个实数域上的积分等于一。通常记作$\delta(\cdot)$.
\end{definition}

狄拉克$\delta$分布也称\keyindex{狄拉克$\delta$函数}{Dirac delta function}{}，
简称\keyindex{$\delta$分布}{delta distribution}{}
或\keyindex{$\delta$函数}{delta function}{}，
它最早由狄拉克\begin{marginfigure}
    \includegraphics[width=\linewidth]{chap07/PaulDirac1933.jpg}
\end{marginfigure}\sidenote{英国物理学家保罗·阿德里安·莫里斯·狄拉克
    (Paul Adrien Maurice Dirac, 1902-1984)。}提出，
...
\begin{definition}
    对于可积函数$f(t)$，其（一元）\keyindex{傅里叶变换}{Fourier transform}{}
    为
    \begin{align}
    ...
    同时，相应的（一元）\keyindex{傅里叶逆变换}{inverse Fourier transform}{}
    为
...
\end{theorem}
\begin{prove}
    取\keyindex{双边指数衰减函数}{two-sided decaying exponential function}{}
...
\begin{theorem}
    ...
    则$f(t)$的\keyindex{直流分量}{DC component}{}
    为
    \begin{align}
...
\begin{theorem}
    对于\keyindex{高斯函数}{Gaussian function}{}
    $f(t)=\mathrm{e}^{-\pi t^2}$，
