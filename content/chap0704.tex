...
如\refvar{7.18}{}所示，扰动的模式比随机模式做得更好，
...
两者都基于称为\keyindex{倒根}{radical inverse}{}
\sidenote{译者注：不确定“radical inverse”一词的翻译，欢迎读者提供帮助。}的构造法，
它基于正整数值$a$可以用下式唯一确定的数字序列$d_m(a)\ldots d_2(a)d_1(a)$
表示为$b$进制\sidenote{译者注：这里$b$称为基（数）（base）。}的事实：
\begin{align}
    \label{eq:7.6}
...
最简单的低偏差序列之一是\keyindex{范德科皮特序列}{van der Corput sequence}{}，
它是由2进制倒根函数给出的1D序列：
\begin{align*}
    x_a=\Phi_2(a)\, .
\end{align*}

\reftab{7.3}展示了范德科皮特序列的前几个值。
...
该问题可以用\keyindex{扰频的}{scrambled}{}Halton和Hammersley序列解决，
其计算倒根时对数字施加了重排。
\begin{align}
    \label{eq:7.8}
...
\reffig{7.26}展示了对Halton序列扰频后的效果。
\begin{figure}[htbp]
    \centering
    ...
    \caption{Halton样本值是否有扰频的图示。(a)在样本向量的高维中，
        ...
        (b)扰频序列即\refeq{7.8}通过随机重排样本索引的数字打破了该结构。}
    \label{fig:7.26}
\end{figure}
...
见\refvar{RadicalInverse}{()}之后的习题\refsub{7.11.3}了解关于2进制情况下的更高效实现的讨论。
\begin{lstlisting}
...
扰频倒根的重排表在所有\refvar{HaltonSampler}{}实例间
...
样本偏移量的计算不考虑随机数字重排，所以它们没有被包含在这里算出的样本值中。
此外，因为前两维数字中较小的\refvar{baseExponents}{[i]}用于选择采样哪个样本，
所以这些数字必须在为样本向量的前两维计算倒根之前就被丢弃掉，
因为方法\refvar[HaltonSampler::SampleDimension]{SampleDimension}{()}应该返回正被采样的像素内的小数偏移量。
更高维则被直接采样，包含随机重排。
\begin{lstlisting}
